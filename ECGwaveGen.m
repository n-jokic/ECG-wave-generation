function [QRSwave] = ECGwaveGen(bpm,duration,fs,amp, PVC, PVCchance)
...
if (exist('PVC') ~= 1)  PVC=  false;   end  
if (exist('PVCchance') ~= 1)  PVCchance = 0.1;   end  %defaultpvc rate
...
while ( t1+60/bpm+1/sample_freq <= duration) %space to insert another qrs pulse in time line
    ...
    if (insertPVC<=PVCchance && t2+tPVC+2/sample_freq <= duration && PVC == true) %enough space to insert PVC
        t1=t2; t2=t1+tPVC;
        i_t1=time2index(t1); i_t2=time2index(t2);
